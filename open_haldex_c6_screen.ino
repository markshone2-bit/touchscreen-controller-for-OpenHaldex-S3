...
// Set this to PROFILE_C6 for OpenHaldex-C6 firmware.
const ControllerProfile ACTIVE_PROFILE = PROFILE_C6;

const char *WIFI_SSID = (ACTIVE_PROFILE == PROFILE_C6) ? "OpenHaldex-C6" : "OpenHaldex-S3";
const char *WIFI_PASS = "";
const char *HALDEX_IP = (ACTIVE_PROFILE == PROFILE_C6) ? "192.168.1.1" : "192.168.4.1";

const char *MODE_API_PATH = "/api/mode";
const char *STATUS_API_PATH = (ACTIVE_PROFILE == PROFILE_C6) ? "/api/dashboard" : "/api/status";

const char *MODE_NAMES_S3[] = {
...
const char *MODE_NAMES_S3_FALLBACK[] = {
...
const char *BUTTON_LABELS_S3[] = {
  "STOCK (FWD)", "90 / 10", "80 / 20",
  "70 / 30", "60 / 40", "50 / 50"
};

const char *BUTTON_LABELS_C6[] = {
  "STOCK", "FWD", "50 / 50",
  "60 / 40", "75 / 25", "EXPERT"
};

const char *MODE_NAMES_C6[] = {
  "STOCK", "FWD", "5050", "6040", "7525", "EXPERT"
};

const uint8_t C6_MODE_VALUES[] = {0, 1, 2, 3, 4, 5};
...
String buildApiUrl(const char *path) {
  return String("http://") + HALDEX_IP + path;
}

const char *profileName() {
  return (ACTIVE_PROFILE == PROFILE_C6) ? "C6" : "S3";
}

const char *buttonLabelForIndex(int idx) {
  if (idx < 0 || idx >= 6) {
    return "?";
  }
  return (ACTIVE_PROFILE == PROFILE_C6) ? BUTTON_LABELS_C6[idx] : BUTTON_LABELS_S3[idx];
}

const char *modeNameForIndex(int idx) {
  if (idx < 0 || idx >= 6) {
    return "?";
  }
  return (ACTIVE_PROFILE == PROFILE_C6) ? MODE_NAMES_C6[idx] : MODE_NAMES_S3[idx];
}
...
void initButtons() {
  ...
  for (int r = 0; r < rows; r++) {
    for (int c = 0; c < cols; c++) {
      ...
      buttons[idx].w = btnW;
      buttons[idx].h = btnH;
      buttons[idx].label = buttonLabelForIndex(idx);
      idx++;
    }
  }
}
...
void drawHeader() {
  ...
  gfx->setTextSize(3);
  gfx->setCursor(22, 16);
  gfx->print("Open Haldex Control");

  gfx->setTextColor(COLOR_TEXT_DIM);
  gfx->setTextSize(2);
  gfx->setCursor(22, 58);
  gfx->print("Audi S3 8L torque split (profile ");
  gfx->print(profileName());
  gfx->print(")");
}
...
void refreshModeConfirmation() {
  ...
  http.begin(buildApiUrl(STATUS_API_PATH));
  ...
  if (ACTIVE_PROFILE == PROFILE_C6) {
    int modeValue = -1;
    if (extractJsonIntField(resp, "mode", modeValue) && modeValue >= 0 && modeValue < 6) {
      lastApiResult = "Controller: ";
      lastApiResult += MODE_NAMES_C6[modeValue];
    }
  } else {
    String mode;
...
static bool postModeName(const char *modeName, int &codeOut, String &respOut) {
  ...
  http.begin(buildApiUrl(MODE_API_PATH));
...
static bool postModeNumber(uint8_t modeValue, int &codeOut, String &respOut) {
  HTTPClient http;
  http.setTimeout(1500);
  http.begin(buildApiUrl(MODE_API_PATH));
  http.addHeader("Content-Type", "application/json");

  String body = String("{\"mode\":") + String(modeValue) + "}";
...
bool sendModeToHaldex(int modeIndex) {
  ...
  if (ACTIVE_PROFILE == PROFILE_C6) {
    ok = postModeNumber(C6_MODE_VALUES[modeIndex], code, resp);
  } else {
    ok = postModeName(MODE_NAMES_S3[modeIndex], code, resp);

    // Some S3 builds accept ratio strings with colon format only.
    if (!ok) {
      int code2 = 0;
      String resp2;
      if (postModeName(MODE_NAMES_S3_FALLBACK[modeIndex], code2, resp2)) {
        code = code2;
        resp = resp2;
        ok = true;
      }
  ...
  if (ok) {
    lastApiResult = "OK " + String(code) + " -> " + String(modeNameForIndex(modeIndex));
    refreshModeConfirmation();
    return true;
  }

  lastApiResult = "HTTP fail " + String(code) + " -> " + String(modeNameForIndex(modeIndex));
